...
% clear act
arduinoObj = serialport("COM3",9600); %%read objectno
...
plot_size=70; %%number of samples in a plot
...
%     end
    ...
    if src.UserData.timer >= src.UserData.timelim  %%only entered after buffer duration
        ...
        if src.UserData.Count < src.UserData.plotsize %%plot after number of samples attained
           plot(src.UserData.Time,src.UserData.RAWData);
        ...
        if src.UserData.Count > src.UserData.plotsize %%plot after number of samples attained
           plot(src.UserData.Time(end-int32(src.UserData.plotsize+1):end),src.UserData.RAWData(end-int32(src.UserData.plotsize+1):end));

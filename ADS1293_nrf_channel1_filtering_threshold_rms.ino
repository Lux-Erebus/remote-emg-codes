//this program does on-board filtering and detects thresholds using RMS
//header files: SPI and Arduino
#include <SPI.h>
#include "Arduino.h"
...
class HighPass
{
  ...
  public:  
    ...
    float filt(float xn){
      ...
      return y[0]; //return the final filtered value
    }
};

// Filter instance
HighPass<2> hp(10,533,0); //Highpass filter of 10Hz, sampling frequency of 533Hz, filter order of 2
...
void loop() {
  ...
  //  transmitData(conv_ych1); //transmit to matlab
    ...
    {
      counter++;
      if(counter%250==0) //blink the LED every 0.5s
      {
    ...
    if(phase==1)
    {
        ...
        phase=2; //change state to signal calculations are completed
...
//        Serial.print(rms);
    ...
    if(phase==2)
    {
      ...
      if(window_counter==rms_size) //find rms after window collected
      {
        ...
        if(rms>(avg+3*std_dev)) //use rms for all comparisons
        {
